		...
		if (this.isSmoking) {
			for (BlockPos blockpos : this.affectedBlockPositions) {
				...
				if (state.getMaterial() != Material.AIR && !state.getBlock().getTranslationKey().contains("grave") && DragonUtils.canDragonBreak(state.getBlock()) && mobGriefing) {
					...
					if (state.getMaterial() == Material.ROCK && (block != ModBlocks.crackledCobblestone && block != Blocks.COBBLESTONE && block != Blocks.MOSSY_COBBLESTONE && block != Blocks.COBBLESTONE_WALL)) {
