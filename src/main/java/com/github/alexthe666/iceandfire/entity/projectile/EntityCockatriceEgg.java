    ...
    public void handleStatusUpdate(byte id) {
        if (id == 3) {
            ...
            if(thrower != null && thrower instanceof EntityPlayer){
                for (int i = 0; i < 8; ++i) {
                    ParticleHelper.spawnParticle(this.world, EnumParticleTypes.HEART, this.posX, this.posY, this.posZ, ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D);
                }
            }
        }
    }
